...
Segmentor::parse_cfg(ltp::utility::ConfigParser & cfg) {
  ...
  if (cfg.has_section("train")) {
    ...
    if (cfg.get_integer("train", "use-natural", intbuf)) {
      if(intbuf==1) {
        train_opt.use_natural = true;
      } 
    } else {
      WARNING_LOG("use-natural is not configed, false is set as default.");
    }

    if (train_opt.use_natural) {
      if (cfg.get("train", "natural-file", strbuf)) {
        train_opt.natural_file = strbuf;
      } else {
        ERROR_LOG("natural-file config item is not found.");
        return false;
      }

      if (cfg.get_integer("train", "natural-iter", intbuf)) {
        train_opt.natural_iter = intbuf;
      } else {
        train_opt.natural_iter = 10;
        WARNING_LOG("natural-iter is not configed, 10 is set as default.");
      }
    }
  ...
  if(train_opt.use_natural) {
    for (int i = 0; i < natural_dat.size(); ++i) {
      ...
      for ( int j = 0; j < inst->size(); ++j) {
        int flag = inst->natural_flag[j];
        if(4==flag) {//s
          flag = 0;
          flag |=1<<(model->labels.index(__s__));
        } else if(5==flag) {
          flag = 0;
          flag |=1<<(model->labels.index(__b__));
          flag |=1<<(model->labels.index(__s__));
        } else if(6==flag) {
          flag = 0;
          flag |=1<<(model->labels.index(__e__));
          flag |=1<<(model->labels.index(__s__));
        }
        inst->natural_flag[j] = flag;
      }
    }
  }
...
Segmentor::build_feature_space(void) {
  ...
  if (train_opt.use_natural) {
    for(int i = 0; i < natural_dat.size(); ++i) {
      ...
      if((i+1) % train_opt.display_interval == 0) {
        TRACE_LOG("[%d] natural instances is extracted.", (i+1));
      }
    }
  }
}
...
void
Segmentor::update_param(Instance * inst, int time, int *feature_group_updated_time, bool use_natural) {
  ...
  if(train_opt.algorithm == "pa") {
    ...
    model->param.add(update_features,time,step);
      ...
      model->param.add(update_features,time,1.);
    }
}
void
Segmentor::train(void) {
  ...
  if (train_opt.use_natural) {
    const char * natural_file = train_opt.natural_file.c_str();
    if(!read_instance(natural_file, true)) {
      ERROR_LOG("Natural file does not exist.");
      return;
    }
  ...
  if (train_opt.algorithm == "mira") {
    ...
    for (int iter = 0; iter < train_opt.max_iter; ++ iter) {
      ...
      for (int i = 0; i < train_dat.size(); ++ i) {
        ...
        update_param(train_dat[i], iter * train_dat.size() + i + 1, feature_group_updated_time);
        if ((i+1) % train_opt.display_interval == 0) {
    ...
    if (train_opt.use_natural) {
      ...
      TRACE_LOG("Start Train with natural instances");

      int dat_size = train_dat.size() + natural_dat.size();
      int natural_size = natural_dat.size();
      int basic_end_time = train_dat.size() * train_opt.max_iter;

      for (int iter = train_opt.max_iter; iter < (train_opt.max_iter + train_opt.natural_iter); ++iter) {

        for(int i = 0; i < natural_dat.size(); ++i) {
          update_param(natural_dat[i],
                       basic_end_time + (iter-train_opt.max_iter) * dat_size + i + 1, 
                       feature_group_updated_time,
                       true);
          if((i+1) % train_opt.display_interval == 0) {
            TRACE_LOG("[%d] natrual instances is trained.", i + 1);
          } 
        }

        for( int i = 0; i < train_dat.size(); ++i) {
          update_param(train_dat[i], 
                       basic_end_time + (iter-train_opt.max_iter) * dat_size + i + 1 + natural_size,
                       feature_group_updated_time
                       );
          if((i + 1 ) % train_opt.display_interval == 0) {
            TRACE_LOG("[%d] instances is trained.", i+1);
        ...
        model->param.flush(basic_end_time + dat_size * (iter - train_opt.max_iter + 1));
        Model * new_model = NULL;
        new_model = erase_rare_features(feature_group_updated_time);
        swap(model, new_model);
